...
function g() {
  ...
  ;;
  "diff")
    git diff origin/$parent_branch..$curr_branch
  ;;
  "patch")
		mkdir patches &>/dev/null
    if [ "$2" != "" ]; then
      git diff origin/$parent_branch..$curr_branch > patches/$2.diff
    else
      git diff origin/$parent_branch..$curr_branch > patches/$curr_branch.diff
    fi
  ;;
  "bug")
    __gbug "${@:2}"
  ...
  ;;
	"changes")
		git diff --name-status $parent_branch..$curr_branch
	;;
  *)
  echo "usage: g diff
